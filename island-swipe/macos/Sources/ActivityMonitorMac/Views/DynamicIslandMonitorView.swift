...
struct DynamicIslandMonitorView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            RoundedRectangle(cornerRadius: layout.cornerRadius, style: .continuous)
                .stroke(
                    accent.opacity(max(0.14, session.dragProgress * 0.45)),
                    lineWidth: 1
                )
        ...
        .shadow(color: accent.opacity(0.16 + session.dragProgress * 0.18), radius: 24, y: 12)
        .shadow(color: .black.opacity(0.46), radius: 26, y: 14)
        .overlay(alignment: .top) {
            Capsule()
                .fill(accent.opacity(0.28))
                .frame(width: layout.width * 0.52, height: 1)
                .blur(radius: 0.3)
                .padding(.top, 0.5)
        }
    ...
    private var islandLayout: IslandLayout {
        ...
        case .notification:
            return IslandLayout(width: min(262, availableWidth), height: 38, cornerRadius: 22, contentPadding: 14)
        case .expanded, .dragging:
            return IslandLayout(
                width: min(280, availableWidth),
                height: 392,
                cornerRadius: 34,
                contentPadding: 18
            )
        case .accepted, .denied:
            return IslandLayout(width: min(214, availableWidth), height: 38, cornerRadius: 22, contentPadding: 12)
        }
    }
...
struct RiskBadge: View {
    ...
    private var tint: Color {
        ...
        case .low:
            return TerminalNoirTheme.lime
        case .medium:
            return TerminalNoirTheme.amber
        case .high:
            return TerminalNoirTheme.red
        }
    }
...
private struct SwipeHintView: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            ...
            Text("LEFT < 90pt = BLOCK · RIGHT > 90pt = ALLOW")
